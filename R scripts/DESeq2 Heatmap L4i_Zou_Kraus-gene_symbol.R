...
res <- res[!is.na(res$padj) & res$padj > 0.5 & res$log2FoldChange>0, ]
...
m_res <- m_res[!is.na(m_res$padj) & m_res$padj < 0.5 & m_res$log2FoldChange>0, ]
...
map_m2h <- unique(rbind(res_ens, res_name))
...
for (i in seq_along(Zou_annot)) {
  ...
  gene_set <- gene_set[gene_set$hsapiens_homolog_associated_gene_name %in% rownames(mat_scaled) &
                         gene_set$hsapiens_homolog_associated_gene_name %in% rownames(zou_mat_scaled) &
                         gene_set$external_gene_name %in% rownames(mESC_mat_scaled), ]
  
  
  sub_L4i <- mat_scaled[gene_set$hsapiens_homolog_associated_gene_name, , drop = FALSE]
  sub_zou <- zou_mat_scaled[gene_set$hsapiens_homolog_associated_gene_name, , drop = FALSE]
  sub_mESC <- mESC_mat_scaled[gene_set$external_gene_name, , drop = FALSE]
  ...
  ordered_gene_set <- gene_set[match(target_order, gene_set$hsapiens_homolog_associated_gene_name), ]
  heatmap_mat_L4i <- heatmap_mat_L4i[ordered_gene_set$hsapiens_homolog_associated_gene_name, , drop = FALSE]
  heatmap_mat_zou <- heatmap_mat_zou[ordered_gene_set$hsapiens_homolog_associated_gene_name, , drop = FALSE]
  heatmap_mat_mESC <- heatmap_mat_mESC[ordered_gene_set$external_gene_name, , drop = FALSE]
  ...
  df_out <- data.frame(
    ...
    zou_annotation  = z,
    zygote  = heatmap_mat_zou[ordered_gene_set$hsapiens_homolog_associated_gene_name, "zygote"],
    twoC    = heatmap_mat_zou[ordered_gene_set$hsapiens_homolog_associated_gene_name, "twoC"],
    fourC   = heatmap_mat_zou[ordered_gene_set$hsapiens_homolog_associated_gene_name, "fourC"],
    eightC  = heatmap_mat_zou[ordered_gene_set$hsapiens_homolog_associated_gene_name, "eightC"],
    ICM     = heatmap_mat_zou[ordered_gene_set$hsapiens_homolog_associated_gene_name, "ICM"],
    E8      = heatmap_mat_L4i[ordered_gene_set$hsapiens_homolog_associated_gene_name, "E8"],
    L4i     = heatmap_mat_L4i[ordered_gene_set$hsapiens_homolog_associated_gene_name, "L4i"],
    WT_mESC     = heatmap_mat_mESC[ordered_gene_set$external_gene_name, "WT"],
    PARPKO_mESC = heatmap_mat_mESC[ordered_gene_set$external_gene_name, "PARPKO"],
    row.names = NULL,
    check.names = FALSE
  )
  ...
  block_h <- unit(0.5 * n_gene[i] , "mm")
  
  ht_zou <- Heatmap(heatmap_mat_zou, name = "Zou",
                    show_row_names = FALSE, show_column_names = (z == 'ICM'), cluster_columns = FALSE, cluster_rows = FALSE, cluster_row_slices = FALSE,
